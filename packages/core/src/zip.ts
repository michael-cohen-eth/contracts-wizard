...
export function zipContracts(mContracts: Contract[]): JSZip {
  const c = mContracts[mContracts.length - 1]!;
  const { transformImport } = withHelpers(c);
  const contractsVariant = c.upgradeable ? '-upgradeable' : '';
  ...
  const printedContracts = printContracts(mContracts);
  zip.file(fileName, printedContracts);
